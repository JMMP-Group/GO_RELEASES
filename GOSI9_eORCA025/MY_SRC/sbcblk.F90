MODULE sbcblk
   ...
   USE sbcapr  , ONLY : apr      ! sea surface pressure (Pa --  N m-2 ) -- JPALM
   ...
   INTEGER FUNCTION sbc_blk_alloc()
      ...
      IF( .NOT. ln_apr_dyn ) THEN
         ALLOCATE( Cd_atm (jpi,jpj), Ch_atm (jpi,jpj), Ce_atm (jpi,jpj), t_zu(jpi,jpj), q_zu(jpi,jpj), &
            ...
            &      apr(jpi,jpj), STAT=sbc_blk_alloc )
      ELSE
         ALLOCATE( Cd_atm (jpi,jpj), Ch_atm (jpi,jpj), Ce_atm (jpi,jpj), t_zu(jpi,jpj), q_zu(jpi,jpj), &
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
   ...
   SUBROUTINE sbc_blk( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 )   apr(:,:) = sf(jp_slp)%fnow(:,:,1) ! add atm pressure field update
                                                                            ! to be passed to air-sea flux calculation
