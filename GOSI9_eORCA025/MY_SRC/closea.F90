...
CONTAINS
   ...
   SUBROUTINE sbc_clo( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj) ::   zdiag2d  ! 2D workspace to output a diagnostic of closea_flux
      !!----------------------------------------------------------------------
      !
      ...
      !
      emp (:,:) = emp (:,:) * tmask(:,:,1)
      zdiag2d(:,:) = zdiag2d(:,:) * tmask(:,:,1) 
      !
      CALL lbc_lnk( 'closea', emp , 'T', 1._wp )
      CALL lbc_lnk( 'closea', zdiag2d , 'T', 1._wp )
      !
      CALL iom_put( 'closea_emp', zdiag2d)
      !
