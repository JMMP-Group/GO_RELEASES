MODULE traqsr
   ...
   USE trc,        ONLY: trn             ! passive tracer array
   USE par_medusa, ONLY: jpmchn, jpmchd  ! MEDUSA chl
   USE sms_medusa, ONLY: scl_chl_qsr,  & ! scaling factor
                         qsr3d, ln_msa_nemo_qsr
#endif
   ...
   REAL(wp), PUBLIC ::   rn_chl_conc  !: Chlorophyll concentration (in mg/m3; for nn_chldta=0)
   ...
   INTEGER, PARAMETER ::   np_BIO     = 4   ! bio-model light penetration -- within PISCES
   INTEGER, PARAMETER ::   np_RGB_med = 5   ! R-G-B     light penetration with Chlorophyll data from MEDUSA model
   !
...
#if defined key_medusa
      REAL(wp), ALLOCATABLE, DIMENSION(:,:,:) :: zchlmed
#endif
      !!----------------------------------------------------------------------
      !
...
!!gm case neuler  not taken into account....
      ...
      CASE( np_BIO )                   !==  bio-model fluxes  ==!
         ...
         END DO
         !
      CASE( np_RGB , np_RGBc, np_RGB_med )         !==  R-G-B fluxes  ==!
         !
         ...
         !
         IF(( nqsr == np_RGBc ) .OR. ( nqsr == np_RGB_med ))THEN          !*  Variable Chlorophyll
            IF( nqsr == np_RGBc ) THEN
               CALL fld_read( kt, 1, sf_chl )         ! Read Chl data and provides it at the current time step

            ELSE
#if defined key_medusa
               ALLOCATE( zchlmed(jpi,jpj,jpk) )
               zchlmed(:,:,:) = scl_chl_qsr * (trn(:,:,:,jpmchn) + trn(:,:,:,jpmchd)) !! medusa chl in mgChl m-3 -- can be scaled from MEDUSA's namelist if required .
#else
               CALL ctl_stop( 'Only use ln_qsr_rgb .AND. nn_chldta == 2 With MEDUSA BGC model.'  )
#endif
            ENDIF
            DO jk = 1, nksr + 1
...
#if defined key_medusa
                     ELSE
                        zchl = MIN( 10. , MAX( 0.03, zchlmed(ji,jj,jk) ) )
#endif
                     ENDIF
         ...
         END DO
#if defined key_medusa
         IF(nqsr == np_RGB_med) DEALLOCATE( zchlmed )
#endif
         !
      ...
      !
#if defined key_medusa
      IF( iom_use('qsr3d') .OR. (ln_msa_nemo_qsr) ) THEN      ! output the shortwave Radiation distribution
#else
...
#if defined key_medusa
         IF(ln_msa_nemo_qsr)  qsr3d(:,:,:) = zetot(:,:,:)
#endif
         DEALLOCATE( zetot ) 
      ENDIF
      !
   ...
   SUBROUTINE tra_qsr_init
      ...
      NAMELIST/namtra_qsr/  sn_chl, cn_dir, ln_qsr_rgb, ln_qsr_2bd, ln_qsr_bio,  &
         &                  nn_chldta, rn_abs, rn_chl_conc, rn_si0, rn_si1
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_qsr in configuration namelist' )
      ...
      IF( ln_qsr_rgb .AND. nn_chldta == 2 )   nqsr = np_RGB_med
      ...
      SELECT CASE( nqsr )
      !                               
      CASE( np_RGB , np_RGBc , np_RGB_med)     !==  Red-Green-Blue light penetration  ==!
         !                             
         ...
         ENDIF
         !
         IF( nqsr == np_RGB_med ) THEN                 ! constant Chl
            IF(lwp) WRITE(numout,*) '   ==>>>   3D Variable Chlorophyll from MEDUSA BGC model  '
         ENDIF
         !
