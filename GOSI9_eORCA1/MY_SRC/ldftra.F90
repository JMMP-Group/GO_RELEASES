...
! TODO : use a version of zRo that integrates over a few time steps ?
   ...
   SUBROUTINE ldf_eiv_dia( psi_uw, psi_vw )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   zw3d_T ! 3D workspace
      !!----------------------------------------------------------------------
      !
...
!!gm      CALL iom_put( "psi_eiv_vw", psi_vw )
      ...
      !
      IF( iom_use('weiv_masstr') .OR. iom_use('weiv_heattr3d') .OR. iom_use('weiv_salttr3d') .OR. iom_use('weiv_heattr') .OR. iom_use('weiv_salttr')) THEN   ! vertical mass transport & its square value
         zw2d(:,:) = rau0 * e1e2t(:,:)
         DO jk = 1, jpk
         ...
         ENDIF  
      ENDIF
      IF( iom_use('weiv_heattr3d') .OR. iom_use('weiv_heattr') ) THEN
        zztmp = 0.5 * rcp 
        zw2d(:,:)   = 0._wp
        zw3d_T(:,:,:)   = 0._wp 
        DO jk = 1, jpkm1
           DO jj = 2, jpjm1
              DO ji = fs_2, fs_jpim1   ! vector opt.
                 zw3d_T(ji,jj,jk) = zw3d(ji,jj,jk) * ( tsn(ji,jj,jk,jp_tem) + tsn(ji,jj,jk+1,jp_tem) )
                 zw2d(ji,jj) = zw2d(ji,jj) + zw3d_T(ji,jj,jk)   
              END DO
           END DO
        END DO
        CALL lbc_lnk( 'ldftra', zw3d_T, 'T', 1. )
        IF( iom_use('weiv_heattr3d')) THEN
           CALL iom_put( "weiv_heattr3d", zztmp * zw3d_T )               ! 3D heat transport in k-direction
        ENDIF
        IF( iom_use('weiv_heattr')) THEN
           CALL iom_put( "weiv_heattr", zztmp * zw2d )                 ! 2D heat transport in k-direction
        ENDIF 
      ENDIF
      IF( iom_use('weiv_salttr3d') .OR. iom_use('weiv_salttr')) THEN
        zztmp = 0.5 * 0.001
        zw2d(:,:)   = 0._wp
        zw3d_T(:,:,:)   = 0._wp   
        DO jk = 1, jpkm1
           DO jj = 2, jpjm1
              DO ji = fs_2, fs_jpim1   ! vector opt.
                 zw3d_T(ji,jj,jk) = zw3d(ji,jj,jk) * ( tsn(ji,jj,jk,jp_sal) + tsn(ji,jj,jk+1,jp_sal) )
                 zw2d(ji,jj) = zw2d(ji,jj) + zw3d_T(ji,jj,jk)   
              END DO
           END DO
        END DO
        ...
        CALL lbc_lnk( 'ldftra', zw3d_T, 'T', 1. )
        IF( iom_use('weiv_salttr')) THEN 
           CALL iom_put( "weiv_salttr", zztmp * zw3d )                 ! 2D salt transport in k-direction
        ENDIF
        IF( iom_use('weiv_salttr3d')) THEN 
           CALL iom_put( "weiv_salttr3d", zztmp * zw3d_T )             ! 3D salt transport in k-direction
        ENDIF
      ENDIF
