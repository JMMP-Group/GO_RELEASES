...
#if defined key_medusa
   INTEGER, PARAMETER ::   jpr_atm_pco2 = 62 ! Incoming atm pCO2 flux
   INTEGER, PARAMETER ::   jpr_atm_dust = 63 ! Incoming atm aggregate dust
...
!       ( see OASIS3 user guide, 5th edition, p39 )
      ...
      !                         ! since fields received, here zqsr_tot,  are not defined with none option
         CALL ctl_stop( 'sbccpl/sbc_cpl_ice_flx: some fields are not defined. Change sn_rcv_qsr value in namelist namsbc_cpl' )
      END SELECT
