...
CONTAINS
   ...
   SUBROUTINE icb_dia_init( )
      ...
      IF( lk_mpp ) THEN
         ...
         ALLOCATE( nsumbuf(7+nclasses) )  ; nsumbuf(:) = 0
   ...
   SUBROUTINE icb_dia( ld_budge )
      ...
      IF( ld_budge ) THEN
         ...
         IF( lk_mpp ) THEN
            ...
            nsumbuf(4:7) = nspeeding_tickets(:)
            DO ik = 1, nclasses
               nsumbuf(7+ik) = nbergs_calved_by_class(ik)
            END DO
            CALL mpp_sum( 'icbdia', nsumbuf(1:nclasses+7), nclasses+7 )
            !
            nbergs_end        = nsumbuf(1)
            nbergs_calved     = nsumbuf(2)
            nbergs_melted     = nsumbuf(3)
            nspeeding_tickets(:) = nsumbuf(4:7)
            DO ik = 1,nclasses
               nbergs_calved_by_class(ik)= nsumbuf(7+ik)
            END DO
            !
            CALL mpp_min( 'icbdia', vel_factor_min, 1 )
         ENDIF
         !
         ...
         IF (nn_verbose_level > 0) THEN
            ...
            WRITE( numicb, '("n speeding tickets by RK4 stage = ",i6,3(",",i6))') (nspeeding_tickets(ik),ik=1,4)
            IF( SUM(nspeeding_tickets) > 0 ) THEN
               WRITE( numicb, '("min velocity reduction factor = ",f12.8)') vel_factor_min
            ENDIF
         ENDIF
         !
         nbergs_start              = nbergs_end
   ...
   SUBROUTINE icb_dia_speed(pvel_factor, pn_stage)
      !!----------------------------------------------------------------------
      !!----------------------------------------------------------------------
      REAL(wp), INTENT(in) ::   pvel_factor   ! factor by which velocity reduced
      INTEGER , INTENT(in) ::   pn_stage  ! which stage of the RK4 calculation are we on
      !
      IF( (.NOT.ln_bergdia) .OR. pn_stage .lt. 1 .OR. pn_stage .gt. 4 )   RETURN
      nspeeding_tickets(pn_stage) = nspeeding_tickets(pn_stage) + 1
      vel_factor_min = MIN(vel_factor_min,pvel_factor)   ! keep track of minimum reduction factor
      !
